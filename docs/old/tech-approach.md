...
> **Archived (CLI / SQLite era).** This document covers the legacy database structure, inverted indexes, normalization, `LibraryTrack`, and `mm` CLI operations. For **current** product direction, see [`top_level_spec.md`](../top_level_spec.md). For **current** technical approach (web app, Docker), see [`tech-approach.md`](../tech-approach.md) at repo root. For archived functional requirements, see [`old_reqs.md`](./old_reqs.md).
...
└── docs/
    ├── top_level_spec.md         # Current product / hosting spec
    ├── tech-approach.md          # Current technical approach (web / Docker)
    ├── old/
    │   ├── old_reqs.md           # Archived functional requirements (CLI MVP era)
    │   └── tech-approach.md      # Archived CLI/SQLite implementation (this file)
