...
class AmazonMusicSearcher:
    ...
    def search_track(self, track: TrackLike) -> list[AmazonSearchResult]:
    ...
    def _parse_search_results(self, html: str, original_query: str) -> list[AmazonSearchResult]:
        ...
        for div in product_divs[:10]:
            try:
                ...
                if result:
                    result.match_score = self._calculate_match_score(result, original_query)
    ...
    def _calculate_match_score(self, result: AmazonSearchResult, query: str) -> float:
        ...
        return float(fuzz.token_sort_ratio(result_str, query.lower()))

    def _search_via_duckduckgo(self, track: TrackLike) -> list[AmazonSearchResult]:
        try:
            from ddgs import DDGS
        except ImportError:
            logger.warning("ddgs not installed; using direct Amazon search")
            return self._search_amazon(self._build_search_query(track))
