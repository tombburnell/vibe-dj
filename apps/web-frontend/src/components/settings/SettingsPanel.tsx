...
/** Full settings form (appearance, Spotify, local scan, playlists). */
  ...
  return (
    <div className="mx-auto max-w-lg space-y-6">
      ...
      <section className="space-y-3 rounded border border-border bg-surface-1 p-4">
        ...
        <p className="text-[0.75rem] text-muted">
          Deleting a playlist removes it from the catalog and unlinks it from every source track
          (tracks themselves are not deleted).
        </p>
        {playlistsQuery.isLoading ? (
          <p className="text-[0.75rem] text-muted">Loading…</p>
        ) : playlistsQuery.error ? (
          <p className="text-[0.75rem] text-red-400">{playlistsQuery.error.message}</p>
              ...
              >
                <div className="min-w-0">
                  ...
                  <div className="text-[0.7rem] text-muted">
                    {p.import_source ?? "—"} · {new Date(p.created_at).toLocaleString()}
                  </div>
                </div>
                <button
                  type="button"
